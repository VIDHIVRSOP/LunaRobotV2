...
async def lego(event):
 ...
 try:
    ...
    fname = "luna.png"
    img.save(fname, "png")
    await telethn.send_file(event.chat_id, file=fname, caption = f"`Created by` [Sofia-Darling💞](https://t.me/Sofia_op_bot)")         
    await pesan.delete()
    if os.path.exists(fname):
            os.remove(fname)
 except Exception as e:
    await event.reply(f'Oops Error, Report [Here](https://t.me/SOFIASUPPORT)')
